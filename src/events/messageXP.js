...
module.exports = {
  ...
  async execute(message) {
    ...
    try {
      const config = anticapsCache.getConfig(
        message.guild.id,
        message.channel.id,
      );
      ...
      if (config) {
        ...
        if (!isAllowed) {
          ...
          if (content.length >= 5) {
            // Chỉ kiểm tra tin nhắn dài hơn 5 ký tự
            const uppercaseChars = content.replace(
            ...
            if (relevantLength > 0 && uppercaseChars / relevantLength > 0.7) {
              // Tăng ngưỡng lên 70% chữ cái in hoa
              await message
                .delete()
                .catch((err) =>
                  Logger.warn(
                    `[AntiCaps] Failed to delete message: ${err.message}`,
                  ),
                );
      ...
      Logger.error(`[AntiCaps Check Error] ${err.message}`, {
        stack: err.stack,
      });
    ...
    if (levelUp.leveledUp) {
      message.channel.send(
        `🎉 <@${user.userId}> đã lên cấp **${levelUp.newLevel}** và nhận **${levelUp.reward.toLocaleString()} VNĐ**!`,
      );
    }
  },
};

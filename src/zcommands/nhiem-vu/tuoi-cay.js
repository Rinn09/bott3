...
const TASK_ID = "tuoiCay"; // Giữ nguyên
const JOB_NAME = "nông dân"; // **Sửa thành chữ thường**

module.exports = {
  data: new SlashCommandBuilder()
    .setName("tuoi-cay")
    .setDescription("Tưới cây để chăm sóc ruộng, nhiệm vụ dành cho nông dân!"),
    ...
    try {
      ...
      if (!jobData) {
        return interaction.editReply(
          `❌ Hệ thống chưa định nghĩa nghề ${JOB_NAME}.`,
        );
      }
      ...
      if (
        !user.mainJob ||
        !user.mainJob.name ||
        user.mainJob.name.trim().toLowerCase() !== JOB_NAME
      ) {
        Logger.warn(
          `[Job Check Failed - ${interaction.commandName}] User: ${userId}, Stored Job: '${user?.mainJob?.name}', Expected: '${JOB_NAME}'`,
        );
        return interaction.editReply({
      ...
      if (!task) {
        return interaction.editReply(
          `❌ Không tìm thấy nhiệm vụ ${TASK_ID} cho nghề ${jobData.name}.`,
        );
      }
      ...
      if (now - lastUsed < cooldownTime) {
        ...
        return interaction.editReply({
          content: `⏳ Hãy đợi **${m} phút ${s} giây** nữa để ${task.name.toLowerCase()} lần tiếp theo.`,
        });
      }
      ...
      Logger.error(`Lỗi lệnh ${TASK_ID}: ${error.message}`, {
        stack: error.stack,
      });
      // Đảm bảo editReply nếu đã defer
      if (interaction.deferred || interaction.replied) {
        await interaction.editReply({
          content: `❌ Có lỗi xảy ra khi thực hiện nhiệm vụ ${task?.name || TASK_ID}.`,
        });
      } else {

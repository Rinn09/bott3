...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('price_per_item')
                ...
                .setRequired(true)
                .setMinValue(0)), // Cho phép bán giá 0 nếu muốn (tặng)
    ...
    async execute(interaction) {
        ...
        const currentListingsCount = await MarketListing.countDocuments({ sellerId, guildId, status: 'active' });
        ...
        try {
            const sellerData = await User.findOne({ userId: sellerId, guildId }).session(session);
            if (!sellerData) {
            ...
            if (itemInInventory < quantityToSell) {
                throw new Error(`Bạn không có đủ ${quantityToSell} vật phẩm \`${itemIdToSell}\` để bán. Số lượng hiện có: ${itemInInventory}.`);
            }

            const shopItemData = await ShopItem.findOne({ itemId: itemIdToSell }).lean(); // .lean() để lấy plain JS object
            if (!shopItemData) {
                throw new Error(`Vật phẩm với ID \`${itemIdToSell}\` không tồn tại trong danh mục hệ thống.`);
            }
            // Kiểm tra xem vật phẩm có được phép bán lại không (nếu có trường `canBeSoldOnMarket` trong ShopItem)
            // if (shopItemData.sellPrice === null && !shopItemData.canBeSoldOnMarket) { // Ví dụ
            //     throw new Error(`Vật phẩm **${shopItemData.name}** không được phép đăng bán trên chợ.`);
            // }
            ...
            const newListing = new MarketListing({
                ...
                status: 'active',
                itemSnapshot: { // Lưu trữ thông tin vật phẩm tại thời điểm đăng
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`Bạn đã đăng bán **${quantityToSell} ${shopItemData.name}** lên chợ với giá **${pricePerItem.toLocaleString()} VNĐ** mỗi cái.`)
                .addFields(
                    { name: '🔎 ID Tin đăng', value: `\`${newListing._id}\`` }, // Dùng để quản lý sau này (ví dụ: hủy tin đăng)
                    { name: '📦 Số lượng còn lại trong túi', value: `${newInventoryQty}` }
                 )
                .setTimestamp()
                .setFooter({ text: `Người bán: ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL() });

            await interaction.editReply({ embeds: [embed] });

        } catch (error) {
            await session.abortTransaction();
            Logger.error(`Lỗi lệnh /market-sell bởi ${interaction.user.tag} (ID: ${sellerId}) cho item ${itemIdToSell}: ${error.message}`, { stack: error.stack });
            await interaction.editReply({ content: `❌ Đã xảy ra lỗi khi đăng bán: ${error.message}` });
        } finally {

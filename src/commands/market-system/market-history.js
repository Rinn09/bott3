...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const generateHistoryEmbedAndButtons = async (page) => {
            const mongoQuery = {
                guildId: guildId,
                $or: [ // Tìm các giao dịch mà người dùng là người mua HOẶC người bán
                    { buyerId: userId },
                    { sellerId: userId }
                ]
            };

            const transactionsCount = await MarketTransaction.countDocuments(mongoQuery);
            const totalPages = Math.ceil(transactionsCount / TRANSACTIONS_PER_PAGE) || 1;
            ...
            const embed = new EmbedBuilder()
                .setTitle(`📜 Lịch sử giao dịch chợ của bạn - Trang ${page}/${totalPages}`)
            ...
            if (!transactions.length) {
                embed.setDescription('Bạn chưa có lịch sử giao dịch nào trên chợ.');
                 const components = transactionsCount > 0 ? [generatePaginationButtons(page, totalPages, "market_history_")] : [];
            ...
            const components = [generatePaginationButtons(page, totalPages, "market_history_")];
        ...
        // Hiển thị lần đầu
        const initialData = await generateHistoryEmbedAndButtons(requestedPage);
        ...
        const filter = i => i.user.id === interaction.user.id && i.customId.startsWith('market_history_');
        ...
        collector.on('collect', async i => {
            ...
            const newData = await generateHistoryEmbedAndButtons(currentCollectorPage);
            await i.editReply(newData);
        });

        collector.on('end', async (collected, reason) => {
             if (reason !== 'messageDelete') {
                const finalData = await generateHistoryEmbedAndButtons(currentCollectorPage);
                ...
                try {
                    // Chỉ edit nếu message còn tồn tại
                     if (message) await message.edit({ components: disabledComponents }).catch(()=>{});
                } catch (error) {

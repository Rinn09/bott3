...
const TASK_ID = 'khamsucKhoe'; // Giữ nguyên
const JOB_NAME = 'bác sĩ'; // **Sửa thành chữ thường**

module.exports = {
  data: new SlashCommandBuilder()
    .setName('kham-suc-khoe') // **Sửa tên lệnh cho đúng với TASK_ID**
    .setDescription('Khám sức khỏe cho người dân, nhiệm vụ dành cho bác sĩ!'), // **Sửa mô tả cho đúng với JOB_NAME**
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (interaction.deferred || interaction.replied) {
         ...
         await interaction.reply({ content: `❌ Có lỗi xảy ra khi thực hiện nhiệm vụ ${task?.name || TASK_ID}.`, ephemeral: false});
      }
    }
  }
};
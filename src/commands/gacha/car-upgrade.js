  ...
  async execute(interaction) {
    ...
    try {
      ...
      try {
        ...
        // Tìm lại carInstance và partInstance trong session
        ...
        if (partToInstallIdx === -1)
          throw new Error("Phụ tùng muốn lắp không còn trong kho.");
        const partInstToInstall = userInSession.garage.parts.find(
          (p) => p._id.toString() === partInstanceIdToInstall,
        );
        if (!partInstToInstall)
          throw new Error("Phụ tùng muốn lắp không còn trong kho.");
        if (partInstToInstall.installedOnCar)
          throw new Error(
            `Phụ tùng ${partDefToInstall.name} đã được lắp trên một xe khác rồi.`,
          );
        if (upgradeCost > 0) {
        ...
        if (oldPartInstanceIdInSlot) {
          const oldPartInst = userInSession.garage.parts.find((p) =>
            p._id.equals(oldPartInstanceIdInSlot),
          );
          if (oldPartInst) {
            oldPartInst.installedOnCar = null; // Đánh dấu là đã tháo
          }
        }

        carInst.installedParts.set(partSlotToUpgrade, partInstToInstall._id);
        partInstToInstall.installedOnCar = carInst._id; // Đánh dấu phụ tùng này đã được lắp

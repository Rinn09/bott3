...
module.exports = {
  ...
  async executeTask(interaction, user, jobDefinition, taskDefinition) {
    const {
      name: taskName,
      xp: taskXpFromDef,
      reward: taskBaseRewardFromDef,
      cooldown: taskCooldownFromDef,
    } = taskDefinition;

    const cooldownTimeMs = taskCooldownFromDef || 5 * 60 * 1000; // 5 phút default
    const xpGain = taskXpFromDef || 20; // 20 XP default
    let moneyEarned = taskBaseRewardFromDef || 0;

    // Logic tính lương theo level nếu reward của task là 0 và job có salaryByLevel
    if (
      moneyEarned === 0 &&
      ...
      // (Ví dụ: task chính là task duy nhất có reward = 0 trong job đó)
      const primarySalaryTask = jobDefinition.tasks.find(
        (t) => t.reward === 0 && t.taskId === this.taskId,
      );
    ...
    const embed = new EmbedBuilder()
      .setTitle(`🛠️ ${taskName} thành công!`)
      .setColor("#20B2AA") // LightSeaGreen - màu cho công nhân
      .setDescription(
